...
<%--<%@ taglib prefix="tag" uri="http://www.db.com"%>--%>
<%@ page contentType="text/html;charset=utf-8" %>

<c:set var="title" value="全部员工"></c:set>
...
<div class="Main">
	...
	<div class="ContentWrapper">
		<div class="MainContent">
			...
			<table border="1" class="Table">
                ...
                <c:forEach items="${map.page.list}" var="emp">
                    <tr>
                        ...
                        <td>
                            <c:if test="${emp.workStatus==null || emp.workStatus==0 }">在职</c:if>
                            <c:if test="${emp.workStatus == 1}">离职</c:if>
                        </td>

                        <td class="ListAction">
                            <summer:a href="/emp/detail?id=${emp.id}" style="ListBtn" body="详情"></summer:a>
                            <summer:a href="/emp/edit?id=${emp.id}" style="ListBtn BtnGhost" body="编辑"></summer:a>
                            <div class="SelectWrapper">
                                ...
                                <select name="" id="" class="CustomSelectText AjaxSelect">
	                                ...
	                                <c:if test="${CurrentUser.check('/emp/updateEmpGroup')}">
		                                <option value="/emp/updateEmpGroup?&empId=${emp.id}" class="OptionRedirect">设置角色权限</option>
	                                </c:if>
	                                ...
	                                <c:if test="${emp.workStatus == 1}">
                                        <option value="/emp/deleteEmp?empId=${emp.id}" data-url="/emp/deleteEmp?empId=${emp.id}" data-msg="删除成功" data-confirm="你确定要删除该员工?" data-pendingType="dialog" data-pending="删除中..." class="AjaxBtnConfirm">删除</option>
	                                </c:if>
                                </select>
                            </div>
                        </td>
			...
			<c:set var="pageUrl" value="/emp/list"></c:set>
