			...
			<table class="Table">
				...
				<tr>
						...
						<div class="CarInfo GoodInfo">
							...
							<div class="CarOtherInfo">
								<div class="CarOtherInfoItem">
									...
									<span>${good.srunKm/10000}万公里</span>
					...
					<td class="CarPriceWrapper">
						...
						<div class="ServicePriceWrapper">服务费：<span class="Price">${goods.servicePrice}元</span>
						</div>
					</td>
					<td style="width: 70px">
					...
					<td class="ActionWrapper">
						...
						<c:if test="${CurrentUser.check('/carGoods/edit')}">
							<a class="ListBtn" href="/carGoods/edit?id=${goods.id}&status=20">编辑</a>
						</c:if>

						<c:if test="${goods.carStatus == 10 && CurrentUser.check('/carGoods/saleOnOff') }">
								<a href="javascript:;" class="ListBtn AjaxBtnConfirm AjaxBtn" data-msg="上架成功"
								   data-confirm="你确定要上架?" data-pending="上架中"
								   data-url="/carGoods/saleOnOff?id=${goods.id}&type=0">上架</a>
							</c:if>
							<c:if test="${goods.carStatus == 20 && CurrentUser.check('/carGoods/saleOnOff')}">
								<%--<a href="javascript:;" class="ListBtn AjaxBtnConfirm AjaxBtn" data-msg="下架成功"--%>
								   <%--data-confirm="你确定要下架?" data-pending="下架中"--%>
								   <%--data-url="/carGoods/saleOnOff?id=${goods.id}&type=1">下架</a>--%>
						...
						<%--<c:if test="${goods.insuranceQuoteStatus == 0 && CurrentUser.check('/carInsurance/noticeQuote')}">--%>
							<%--<div class="NotInsuranceQuoteBtnWrapper">--%>
								<%--<a class="ListBtn NotInsuranceQuote InsuranceQuoteBtn" data-carid="${goods.id}"--%>
								   <%--data-dialog=".DialogNotInsuranceQuote" href="javascript:;"--%>
								   <%--data-cartitle="${goods.carTitle}">车险未报价</a>--%>
								<%--<select>--%>
						...
						<%--<c:if test="${goods.insuranceQuoteStatus == 1 && CurrentUser.check('/carInsurance/noticeQuote')}">--%>
							<%--<a class="ListBtn OnInsuranceQuote InsuranceQuoteBtn"--%>
							   <%--data-dialog=".DialogOnInsuranceQuote" data-cartitle="${goods.carTitle}"--%>
							   <%--data-carid="${goods.id}" href="javascript:;">车险报价中</a>--%>
						<%--</c:if>--%>
						<%--<c:if test="${goods.insuranceQuoteStatus == 2 && CurrentUser.check('/carInsurance/noticeQuote')}">--%>
							<%--<a class="ListBtn HasInsuranceQuote InsuranceQuoteBtn"--%>
							   <%--data-dialog=".DialogHasInsuranceQuote" data-carid="${goods.id}"--%>
							   <%--href="javascript:;">车险已报价</a>--%>
						<%--</c:if>--%>
					</td>
				</tr>
				</c:forEach>
